...
enum pjmedia_rtp_pt
{
    ...
    PJMEDIA_RTP_PT_CODEC2 = 35,       /**< codec2             */
    PJMEDIA_RTP_PT_RAW = 36,       /**< raw             */
...
// anhnh56 20180501 support non-integer frame size
#if defined(PJMEDIA_SUPPORT_FLOAT_PTIME) && (PJMEDIA_SUPPORT_FLOAT_PTIME != 0)
       float frm_ptime;        /**< Decoder frame ptime in msec.   */
       float enc_ptime;        /**< Encoder ptime, or zero if it's
                         equal to decoder ptime.     */
#else //PJMEDIA_SUPPORT_FLOAT_PTIME
       pj_uint16_t frm_ptime;		/**< Decoder frame ptime in msec.   */
       pj_uint16_t enc_ptime;		/**< Encoder ptime, or zero if it's
					     equal to decoder ptime.	    */
#endif //PJMEDIA_SUPPORT_FLOAT_PTIME
