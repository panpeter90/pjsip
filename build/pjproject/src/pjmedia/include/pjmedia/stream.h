...
typedef pj_status_t(* vt_pjmedia_receive_stream_cb) (char *in_data, pj_ssize_t *in_size,char *out_data, pj_ssize_t *out_size,pj_status_t previous_status);

PJ_DEF(pj_status_t)
vt_set_cb_stream_pjmedia(vt_pjmedia_receive_stream_cb receive_function);

PJ_DEF(pj_status_t)
vt_get_change_codec_bitrate(pjmedia_stream *stream, int bitrate);
//MinhBQ
typedef pj_status_t (*vt_pjmedia_receive_stream_report_cb)(unsigned frame_lost);

PJ_DEF(pj_status_t)
vt_set_cb_report_frame_cnt(vt_pjmedia_receive_stream_report_cb
report_function);

//khanhtx3: refs #8179 - use rtcp for network quality
typedef pj_status_t (*vt_pjmedia_rtcp_rx_cb)(void);

PJ_DEF(pj_status_t)
vt_set_cb_rtcp_rx(vt_pjmedia_rtcp_rx_cb report_function);

typedef pj_status_t (*vt_pjmedia_rtcp_tx_cb)(void);

PJ_DEF(pj_status_t)
vt_set_cb_rtcp_tx(vt_pjmedia_rtcp_tx_cb report_function);
